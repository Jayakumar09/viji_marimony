...
const Profile = () => {
  ...
  const handleUpdateSubscription = async (tier) => {
    ...
    try {
      ...
      const orderData = await paymentService.createSubscriptionOrder(tier);
      ...
      paymentService.openRazorpayCheckout(
        ...
        (result) => {
          ...
          setProfileData(prev => ({
            ...
            subscriptionStart: result.subscription?.startDate,
            subscriptionEnd: result.subscription?.endDate,
            isPremium: true
          }));
          setSuccess(`Subscription updated to ${tier} successfully!`);
          toast.success(`Payment successful! Subscription updated to ${tier}!`);
          setUploading(false);
        },
        (error) => {
          // Payment failure callback
          setError(error.message || 'Payment failed or cancelled');
