...
const initiatePayment = async (req, res) => {
  try {
    ...
    const validPaymentModes = ['PAY_PAGE', 'UPI', 'CARD', 'WALLET', 'NET_BANKING', 'BANK_TRANSFER'];
    if (!validPaymentModes.includes(paymentMode)) {
      return res.status(400).json({
        success: false,
        error: `Invalid payment mode. Available modes: ${validPaymentModes.join(', ')}`
      });
    }

    // Validate type
    const validTypes = ['SUBSCRIPTION', 'VERIFICATION'];
    if (!validTypes.includes(type)) {
      return res.status(400).json({
        success: false,
        error: `Invalid type. Available types: ${validTypes.join(', ')}`
      });
    }
...
const handleCallback = async (req, res) => {
  try {
    // Security: Log only essential callback info, not full headers/body which may contain sensitive data
    ...
    // Get raw body for signature verification (stored by express.raw middleware)
