 ...
 * Create a new payment order (verification or subscription)
 */
const createOrder = async (req, res) => {
  try {
    const { type = 'VERIFICATION', verificationType, planId, currency = 'INR' } = req.body;
    ...
    if (type === 'SUBSCRIPTION') {
      ...
      if (!planId) {
        return res.status(400).json({ error: 'Plan ID is required for subscription payment' });
      }
      result = await paymentService.createSubscriptionPayment({
        userId,
        planId,
        currency
      });
    } else {
      // Create verification payment (default)
      if (!verificationType) {
        return res.status(400).json({ error: 'Verification type is required' });
      }
      result = await paymentService.createVerificationPayment({
        userId,
        verificationType,
        currency
      });
...
const verifyPayment = async (req, res) => {
  try {
    const { razorpay_order_id, razorpay_payment_id, razorpay_signature, razorpayOrderId, razorpayPaymentId, razorpaySignature, type } = req.body;
    ...
    // Check if this is a subscription payment by looking up the payment record
    console.log('Looking up payment with orderId:', orderId);
    const paymentRecord = await prisma.payments.findFirst({
      where: { orderId: orderId }
    });
    console.log('Payment record found:', paymentRecord ? paymentRecord.id : 'null');

    let result;
    if (paymentRecord || type === 'SUBSCRIPTION') {
      result = await paymentService.verifyAndCompleteSubscriptionPayment({
        razorpayOrderId: orderId,
        razorpayPaymentId: paymentId,
        razorpaySignature: signature
      });
    } else {
      result = await paymentService.verifyAndCompletePayment({
        razorpayOrderId: orderId,
        razorpayPaymentId: paymentId,
        razorpaySignature: signature
      });
