...
const rateLimitPaymentAttempts = (maxAttempts = 5, windowMs = 60000) => {
  return (req, res, next) => {
    ...
    if (!paymentRateLimit[userId]) {
      paymentRateLimit[userId] = [];
    }
    
    // Remove old attempts outside the window
    paymentRateLimit[userId] = paymentRateLimit[userId].filter(
      timestamp => timestamp > windowStart
    );

    // Check if limit exceeded
    if (paymentRateLimit[userId].length >= maxAttempts) {
      return res.status(429).json({
        error: 'Too many attempts',
        message: 'Please wait before making another payment attempt',
        retryAfter: Math.ceil((paymentRateLimit[userId][0] + windowMs - now) / 1000)
      });
    }

    // Record this attempt
    paymentRateLimit[userId].push(now);
    next();
  };
};
