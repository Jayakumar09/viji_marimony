...
const initiatePayment = async (params) => {
  ...
  const merchantOrderId = `ORD_${Date.now()}_${crypto.randomBytes(8).toString('hex')}`;
  ...
  try {
    ...
    const { orderId, state, redirectUrl, paymentFlowUrl } = response.data;

    if (!orderId) {
      throw new Error('No orderId in PhonePe response');
    }

    // PhonePe Checkout v2 returns the checkout URL in the response
    // Use redirectUrl or paymentFlowUrl from response, or construct as fallback
    let checkoutUrl;
    if (redirectUrl) {
      checkoutUrl = redirectUrl;
    } else if (paymentFlowUrl) {
      checkoutUrl = paymentFlowUrl;
    } else {
      // Fallback: construct URL using the API base URL (same domain as API)
      const baseUrl = phonepeConfig.getBaseUrl();
      checkoutUrl = `${baseUrl}/checkout/v2/pay?orderId=${orderId}`;
    }
    ...
    const payment = await prisma.payments.create({
      data: {
        ...
        phonepeOrderId: orderId, // Store in dedicated column for reliable lookups
...
const verifyCallbackSignature = (payload, signature) => {
  ...
  try {
    ...
    // Check length match before timing-safe comparison (timingSafeEqual throws on length mismatch)
    if (signatureBuffer.length !== expectedBuffer.length) {
      console.error('PhonePe callback signature length mismatch');
      return false;
    }
...
const handleCallback = async (callbackData, rawPayload = null, signature = null) => {
  ...
  // Find payment by PhonePe order ID using dedicated column (reliable lookup)
  ...
  // Fallback: Try to find by merchant order ID if not found by PhonePe order ID
  if (!payment) {
    console.log('Payment not found by phonepeOrderId, trying merchant orderId');
  ...
  // Use transaction for atomic payment + subscription updates
  // This ensures data consistency - if subscription creation fails, payment won't be marked successful
...
const getBankTransferDetails = async (params) => {
  ...
  // Generate reference ID using cryptographically secure random bytes
  const referenceId = `VBM-${new Date().toISOString().slice(0, 10).replace(/-/g, '')}-${crypto.randomBytes(3).toString('hex').toUpperCase()}`;
