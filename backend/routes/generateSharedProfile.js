...
function extractCloudinaryPublicId(cloudinaryUrl) {
  try {
    ...
    publicId = publicId.replace(/\.(jpg|jpeg|png|gif)$/i, '');
...
router.get('/:userId/pages', async (req, res) => {
  try {
    ...
    // Get profile photo - try to get from user.photos if it's a valid Cloudinary URL
    // Falls back to profile_photo field but only if it's a valid URL
    let profilePhotoUrl = user.profile_photo;
    if (user.photos) {
      try {
        const parsedPhotos = JSON.parse(user.photos);
        if (Array.isArray(parsedPhotos) && parsedPhotos.length > 0) {
          // Find first Cloudinary URL for profile photo
          const cloudinaryPhoto = parsedPhotos.find(p => 
            p && typeof p === 'string' && p.includes('cloudinary')
          );
          if (cloudinaryPhoto) {
            profilePhotoUrl = cloudinaryPhoto;
          }
        }
      } catch {}
    }
...
router.get('/:userId', async (req, res) => {
  try {
    ...
    // Get profile photo and gallery - only from Cloudinary URLs in user.photos
    let profilePhotoUrl = user.profile_photo;
    let gallery = [];
    if (user.photos) {
      try {
        ...
        if (Array.isArray(parsedPhotos) && parsedPhotos.length > 0) {
          ...
          // First Cloudinary photo is profile photo
          if (cloudinaryPhotos.length > 0) {
            profilePhotoUrl = cloudinaryPhotos[0];
            // Rest are gallery (excluding profile photo)
            gallery = cloudinaryPhotos.slice(1);
          }
        }
      } catch {}
    }
